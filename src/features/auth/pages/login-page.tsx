    ...
    <div className="flex w-full flex-col sm:max-w-sm md:max-w-md lg:max-w-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-col gap-y-6 pt-12">
          <Input autoFocus errorMessage={errors.email?.message} icon={AtSign} placeholder="masukkan email anda" {...register("email")} />
          <Input errorMessage={errors.password?.message} icon={Lock} placeholder="masukkan password anda" type="password" {...register("password")} />
        </div>
        <div className="mt-12">
      ...
      <div className="mt-6">
        <p className="text-center text-gray-400">
          ...
          <Link className="font-semibold text-primary transition-colors duration-300 ease-in-out hover:text-primary/40" to="/register">
            di sini
          </Link>
        </p>
      </div>
