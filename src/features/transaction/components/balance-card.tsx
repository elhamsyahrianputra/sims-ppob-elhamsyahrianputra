    ...
    <div className="flex flex-1 flex-col rounded-lg bg-[url(/assets/bg/Background-Saldo.png)] bg-cover bg-right bg-no-repeat md:rounded-xl">
      <div className="flex h-full flex-col justify-between p-5 text-white md:p-6">
        ...
        <div className="flex gap-x-1.5 font-bold text-semibold">
          ...
          <span className="text-3xl leading-7 tracking-wider md:text-4xl">{showBalance ? formatRupiah(balance) : "•••••••"}</span>
        </div>
        <button className="inline-flex w-fit cursor-pointer bg-transparent py-1 pr-7 text-xs transition-colors hover:opacity-80" onClick={() => setShowBalance(!showBalance)} type="button">
